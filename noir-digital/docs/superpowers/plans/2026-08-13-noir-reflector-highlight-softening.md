# NOIR Reflector Highlight Softening Implementation Plan
...
**Architecture:** Keep the current PMREM reflector environment and alter only its three fixed reflector intensity constants. Protect the exact values with the existing environment configuration test, then validate the production Canvas UI variant visually.

**Tech Stack:** TypeScript, Three.js, React Three Fiber, Vitest, Biome, Next.js

---

### Task 1: Reduce fixed reflector intensity

**Files:**
- Modify: `scene/hero-canvas-ui-environment.test.ts`
- Modify: `scene/hero-canvas-ui-glass-config.ts`

- [ ] **Step 1: Write the failing test**

Update the reflector intensity expectation in `scene/hero-canvas-ui-environment.test.ts` to require the softened values:
...
```powershell
npx vitest run scene/hero-canvas-ui-environment.test.ts
```

Expected: FAIL because the current intensities remain `78`, `62.4`, and `90`.

- [ ] **Step 3: Apply the isolated configuration change**

In `scene/hero-canvas-ui-glass-config.ts`, retain every reflector property except the three intensities:
...
npx biome check scene/hero-canvas-ui-environment.test.ts scene/hero-canvas-ui-glass-config.ts
npm run typecheck
npm run build
```
...
```powershell
git add -- scene/hero-canvas-ui-environment.test.ts scene/hero-canvas-ui-glass-config.ts
git commit -m "fix(hero): soften fixed glass reflection"
```
