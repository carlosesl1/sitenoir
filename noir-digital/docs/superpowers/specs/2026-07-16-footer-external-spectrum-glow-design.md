...
The footer icon remains readable as polished chrome. A compact rainbow halo sits behind the model and follows only its outer projected contour. The model surface and enclosed openings receive no spectral fill. Open concavities that connect to the exterior remain exterior; only fully enclosed openings are filled when creating the silhouette mask.
